...
class TriMesh:
    ...
    def write_ply(self, raw_f: BinaryIO):
        write_ply(
            raw_f,
            coords=self.verts,
            rgb=(np.stack([self.vertex_channels[x] for x in "RGB"], axis=1) if self.has_vertex_colors() else None),
            faces=self.faces,
        )
    ...
    def write_obj(self, raw_f: BinaryIO):
        ...
        faces = ["f {} {} {}".format(str(tri[0] + 1), str(tri[1] + 1), str(tri[2] + 1)) for tri in self.faces.tolist()]
